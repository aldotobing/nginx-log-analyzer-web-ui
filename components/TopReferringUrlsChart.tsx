...
interface TopReferringUrlsChartProps {
  fetchData: () => Promise<Record<string, number>>; // Function to fetch data
}
...
export function TopReferringUrlsChart({
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const result = await fetchData(); // Fetch data from parent-provided function
        setData(result);
      } catch (err) {
  ...
  const sortedData = Object.entries(data)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10); // Top 10 Referring URLs
    ...
    datasets: [
      {
        ...
        borderWidth: 2,
        hoverBackgroundColor: "rgba(75, 192, 192, 0.9)", // Aqua Blue on Hover
      },
    ],
  };
